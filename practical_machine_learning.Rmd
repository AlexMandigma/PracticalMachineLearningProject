...
We will first predict the outcomes using trees and use cross validation 10 times.
```{r}
control <- trainControl(method = "cv", number = 10)
model_fit <- train(classe ~ ., data = data_train, method = "rpart", 
                   trControl = control)
...
prediction_tree <- predict(model_fit, data_test)
(confusionmatrix_tree <- confusionMatrix(data_test$classe, prediction_tree))
```
...
```
We can see that the model has a 48.51% accuracy of predicting the outcome based on the confusion matrix which is not actually a good indicator. The plot above also shows the predictions vs. the observed outcome,
